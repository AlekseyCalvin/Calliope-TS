...
function flattenSyllables(words: ClsWord[], ius?: IntonationalUnit[]): FlatSyl[] {
  ...
  for (let i = 0; i < result.length; i++) {
    // Clitics ('x') promote too — the canonical Attridge example is exactly a
    // clitic in a weak trough carrying the beat ("In the FORests OF the NIGHT"):
    // promotion is licensed by the trough, not the syllable's citation class.
    // The clitic's promoted beat scores LOWER than a 'w' promotion (see
    // scoreSyllable), so it never outbids an honest realisation.  A clitic at
    // the LINE EDGE never promotes (a line-initial "The"/"And" is an upbeat,
    // not a beat) — the edge counts as a weak flank only for 'w' candidates.
    const flanksWeak = weakOrEdge(i - 1) && weakOrEdge(i + 1);
    result[i].promotable =
      (result[i].stress === 'w' && flanksWeak) ||
      (result[i].stress === 'x' && i > 0 && i < result.length - 1 && flanksWeak);
  }
...
function scoreSyllable(syl: FlatSyl, expected: 'W' | 'S'): number {
  ...
  if (expected === 'S') {
    ...
    if (a === 'n') {
      ...
      // The 'n' tier is by construction ACCENT-CAPABLE (wh-words, quantifiers,
      // demonstratives, negators, focus particles — Krifka (13): wh/quantifier
      // words take accents like content).  A function-n must not score BELOW
      // its own weak-slot value (1.6) so steeply that the fitter prefers to
      // waste every promoted opening beat — that asymmetry (0.8 here vs 1.6 at
      // W) made rising readings structurally unbeatable on lines like "WHAT
      // immortal HAND or EYE".
      return syl.isContent ? 1.5 : 1.3;
    }
    ...
    // UNLESS the clitic sits in a genuine weak trough, where Attridge promotion
    // realises the beat on it ("In the FORests OF the NIGHT" — the textbook
    // case).  Priced below the 'w' promotion (0.3): the weakest legal beat.
    if (a === 'x') return syl.promotable ? 0.0 : -3.2;
    // Line-final 'w' beat: the cadence position freely promotes a weak
    // monosyllable — the ballad's wrenched-rhyme convention ("wherefore
    // stopp'st thou ME", "for His civiliTY"): rhyme-crowned closes want ictus.
    // Below the line-final 'n' (2.2), above the mid-line promotion (0.3).
    if (syl.isLineFinal) return 1.5;
...
interface FootCtx {
  isStart: boolean; caesuraBefore: boolean; isEnd: boolean;
  syls: FlatSyl[];                   // the syllables this foot would cover
}
interface Template {
  pattern: ('W' | 'S')[];
...
const P = {
  ...
  TRI_IN_DUPLE: -3.2, // anapest/dactyl substituting inside a duple meter.
                      // NB: sized against the substitution's hidden GAIN — a WWS
                      // foot absorbs an extra weak syllable at full weak credit
                      // (~2.2), so at the old −2.2 the substitution was net-free
                      // and duple fits swallowed ternary feet wholesale (the
                      // "fire of thine" squeeze); −3.2 restores a real cost while
                      // keeping the occasional genuine substitution affordable.
  ...
  ANAC2: -1.2,      // double anacrusis upbeat.  Slightly cheaper than a pyrrhic:
                    // Hiawatha's "By the | SHORES of…" needs it to stay trochaic
                    // tetrameter.  The trochee-eats-rising-lines risk ("and he
                    // STOPpeth ONE of THREE") is contained by the edge-licensed
                    // anapest substitution + rising onset cue on the iambic side.
  ...
  switch (meter) {
    case 'iambic':
      ...
      t = [
        ...
        // Anapestic substitution: LICENSED at the line's edges (the ballad
        // opening "and he STOP|peth ONE…" and cadence "and GLIT|tering EYE" —
        // Tarlinskaja: extra slack clusters at line edges), MARKED mid-line
        // (the "fire of thine" squeeze).  The licence requires GENUINE slack:
        // if a weak slot would bury an accent-capable syllable ('n'+ — "IN what
        // DIStant…", the fronted-focus beat), the substitution is hiding a beat,
        // not absorbing slack, and pays the full price.
        { pattern: ['W', 'W', 'S'], score: c =>
            ((c.isStart || c.isEnd || c.caesuraBefore) &&
             c.syls[0].stress !== 'n' && c.syls[1].stress !== 'n')
              ? -1.8 : P.TRI_IN_DUPLE },
    ...
    case 'trochaic':
      t = [
        ...
        // Dactylic substitution: licensed at the edges (mirror of the iambic
        // anapest licence — same genuine-slack condition), marked mid-line.
        { pattern: ['S', 'W', 'W'], score: c =>
            ((c.isStart || c.isEnd || c.caesuraBefore) &&
             c.syls[1].stress !== 'n' && c.syls[2].stress !== 'n')
              ? -1.8 : P.TRI_IN_DUPLE },
...
function fitMeter(syls: FlatSyl[], meter: MetreName): FitResult {
  ...
  function solve(i: number): Memo {
    ...
    for (const tmpl of templates) {
      ...
      let footScore = tmpl.score({ isStart, caesuraBefore, isEnd, syls: syls.slice(i, i + L) });
...
function onsetBonus(flat: FlatSyl[], meter: MetreName): number {
  // A line-INITIAL 'n' counts as a falling-compatible opening: the n tier is
  // accent-capable by construction (a fronted-focus preposition, a wh-word —
  // "IN what distant deeps", "WHAT the hand"), so a line that opens on one is
  // opening on a light beat, not on an upbeat — PROVIDED the next real beat
  // (m/s) sits an EVEN number of syllables away, keeping the duple alternation
  // ("WHAT the HAND" ✓; but "What had I GIVen" puts the beat 3 off, so the
  // opening wh is an upbeat there and the line stays rising).
  if (flat.length > 0 && flat[0].stress === 'n') {
    let f1 = -1;
    for (let i = 1; i < flat.length; i++) { if (isStrong(flat[i].stress)) { f1 = i; break; } }
    if (f1 < 0 || f1 % 2 === 0) {
...
export function applyStanzaConsensus(
  ...
  if (max < 2) {
    // NO name reaches even a two-line plurality — the scattered-quatrain case
    // (the Mariner opening named iamb/troch/anap/dact, one line each), where the
    // plurality vote deadlocks precisely because every line wandered.  Fall back
    // to EVIDENCE MASS: the meter with the greatest total ranking score across
    // the stanza — accepted only when that reading is genuinely competitive for
    // the stanza as a whole (its mass ≥ 90% of the sum of each line's own best).
    // A truly polymetric or accentual stanza scatters far below this bar, and
    // the rhythm-layer gate still blocks any renaming there.
    if (details.length < 3) return;
    const names = new Set<string>();
    let ownMass = 0;
    for (const d of details) {
      for (const r of d.ranking ?? []) names.add(r.meter);
      ownMass += d.ranking?.[0]?.score ?? 0;
    }
    let bestMass = 0;
    for (const name of names) {
      const m = mass(name);
      if (m > bestMass) { bestMass = m; dominant = name; }
    }
    if (!dominant || bestMass < ownMass * 0.9) return;
  ...
  for (const d of details) {
    ...
    const siblings = (TERNARY_METERS.has(d.meterName as MetreName)
      ...
      // Same-POLARITY families share their upbeat structure: an "anapestic"
      // ballad line IS an iambic line with licensed extra slack (and GLIT|
      // tering EYE), a "dactylic" one a trochee with the same — the near-tie
      // between them is naming noise in exactly the sibling sense.
      || (RISING_METERS.has(d.meterName as MetreName)
        && RISING_METERS.has(dominant as MetreName))
      || (FALLING_METERS.has(d.meterName as MetreName)
        && FALLING_METERS.has(dominant as MetreName));
