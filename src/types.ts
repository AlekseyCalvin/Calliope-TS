...
export interface ClsWord {
  ...
  rhymeFocal?: boolean;              // a line-final FUNCTION word that carries a strong end-rhyme
                                      // with a stressed (content) line-final elsewhere in the stanza
                                      // ("…one of THREE" / "…stopp'st thou ME") — the rhyme template
                                      // transfers a beat.  Set only by the stanza rhyme-fellow pass
                                      // (analyzeStanzas / analyzeReadingDocument), never single-line.
  acuteSyllable?: number;            // syllable index carrying the poet's explicit ACUTE diacritic
                                      // ("Milésien") — an overriding stress cue: the lexical primary
                                      // moves here, and the surface grade never sinks below 'm'.
                                      // Set by assignLexicalStress; honoured at the very end of
                                      // applySurfacePostProcessing, after every demotion pass.
}
...
export interface LineResult {
  ...
  // opinion: its own dictionary, its own syllabifier, its own two iambic
  // algorithms and its own anapestic engine, working ONLY from the raw line
  // text (src/scandroidNative/).  Never derived from anything above.  This is
  // the Calliope (main) pipeline's field.
  scandroidNative?: import('./scandroidNative/engine.js').ScandroidNativeLineResult;
  // Clio (the frozen legacy pipeline, src/clio/) still derives its Scandroid
  // comparison from Calliope-style stress grades via its own untouched
  // src/clio/scandroid.ts — kept here only so Clio's frozen code continues to
  // compile; the main pipeline never sets these.
