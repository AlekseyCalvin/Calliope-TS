...
function isCompound(dep: ClsWord, head: ClsWord): boolean {
  ...
  if ((dep.canonicalRel ?? '') === 'CASE') return false;
...
function combineDecision(
  ...
  switch (rel) {
    ...
    case 'ADVMOD':
      ...
      if (/^(JJ|RB)/.test(head.lexicalClass)) return 'head';
...
export function computePhraseStress(sent: ClsSentence): void {
  ...
  // (which reads the LOCAL minimum) is unaffected.
  {
    ...
    if (ranked.length > 1) {
      ...
      for (let i = 0; i < atMin.length - 1; i++) atMin[i].phraseStress = min + 1;
    }
  }
}
...
function phiProjects(
  ...
  switch (dep.canonicalRel ?? '') {
    case 'CCOMP': case 'XCOMP':
      ...
      if (isMarginalModalUse(head, words)) return false;
      ...
      if (isSemiModalHaveTo(head, words)) return false;
      return true;
    case 'ADVCL': case 'CONJ':
      return true;
    case 'ACL':
