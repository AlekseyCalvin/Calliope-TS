...
export interface LineRhyme {
  endWord: string;
  letter: string;          // scheme key ('A', 'B', … 'Z', 'AA', 'AB', …; '·' = unrhymed)
  type?: RhymeTypeName;    // relation to the matched earlier line
  matchedLine?: number;    // 0-based index within the array passed to detectScheme —
                           // poem-wide when called via applyRhymeAndForm (the normal path)
}

/** Scheme key: 0..25 -> 'A'..'Z'; 26+ -> bijective base-26 multi-letter keys
 *  ('AA','AB',…,'AZ','BA',…) — spreadsheet-column style.  Unbounded: never
 *  collides and never returns '·' (that stays the dedicated unrhymed marker),
 *  so a poem with more than 26 rhyme groups keeps distinct keys instead of
 *  folding unrelated groups together. */
export function rhymeKey(k: number): string {
  let n = k + 1;
  let out = '';
  while (n > 0) {
    const rem = (n - 1) % 26;
...
/** A poem-scoped key allocator: walks rhymeKey(0…∞), skipping every key
 *  already `used` (seeded by the caller) or already handed out by this
 *  allocator itself — so internal-rhyme / caesural / head lettering can draw
 *  fresh keys that never collide with the end-rhyme scheme OR each other, at
 *  any poem length (no cap, no modulo wraparound). */
export interface KeyAllocator { next(): string; }
export function makeKeyAllocator(used?: Set<string>): KeyAllocator {
  const seen = new Set(used ?? []);
  let code = 0;
  return {
    next(): string {
      let key: string;
      do { key = rhymeKey(code++); } while (seen.has(key));
      seen.add(key);
      return key;
    },
  };
...
export function detectScheme(endWords: string[], endTails?: string[][]): LineRhyme[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if ((memberCount.get(r) ?? 0) >= 2) {
      if (!letterOf.has(r)) letterOf.set(r, rhymeKey(k++));
...
/** Display string for a scheme: '·' marks unrhymed.  Single-char keys join
 *  tight, the familiar look ("A·ABCC·B"); once ANY key runs multi-char (26+
 *  distinct groups) every token is space-separated instead, so a two-letter
 *  key never fuses with a neighbouring one ("A · A B … AA …"). */
const schemeStr = (rs: LineRhyme[]): string => {
  const tokens = rs.map(r => r.letter);
  return tokens.some(t => t.length > 1) ? tokens.join(' ') : tokens.join('');
};

/** Canonical scheme for FORM matching ONLY — never displayed to the reader
 *  (schemeStr above is the real, poem-wide display string).  Every line gets
 *  a letter in sequential first-appearance order, unrhymed lines (·) each
 *  their own — so "·A·A" compares as "ABCB" against fixed patterns like
 *  "ABABCDCDEFEFGG". */
function canonicalScheme(rs: LineRhyme[]): string {
  const remap = new Map<string, string>();
  let k = 0;
  const next = () => rhymeKey(k++);
  let out = '';
  for (const r of rs) {
 ...
 *  `globalRhymes` is the SAME poem-wide detectScheme() result the per-stanza
 *  `rhymes` slices were cut from (applyRhymeAndForm) — reused here rather than
 *  re-detected, since re-running detectScheme over the identical flattened
 *  endWords in the identical order could only reproduce the identical letters. */
function poemForm(
  stanzas: { rhymes: LineRhyme[]; details: PhonologicalScansionDetail[] }[],
  globalRhymes: LineRhyme[],
): string | undefined {
  if (globalRhymes.length === 14) {
    const s = canonicalScheme(globalRhymes);
    ...
    if (s.endsWith('GG') || /(..)\1*..$/.test(s)) {
      ...
      const last2 = globalRhymes[12].letter !== '·' && globalRhymes[12].letter === globalRhymes[13].letter;
...
//
// The poem-wide END-rhyme scheme (detectScheme) is the primary, UNTOUCHED
...
// — for an internal-only pair — a fresh key that does not collide with the
// POEM's end letters or any internal-only key already handed out to an
// earlier stanza (the shared `allocator`, see applyRhymeAndForm).  Internal
// rhymes render parenthesised before the end letter ("(A)B"), each carrying
// its own type.  Purely additive: the end scheme's letters and types are
// never modified.

/** Attach pre-caesural internal rhymes to each line's `detail.rhyme`, on top of
 *  this stanza's SLICE of the poem-wide end scheme `rhymes` (from
 *  detectScheme).  `allocator` is shared across the whole poem (one call per
 *  stanza, same allocator instance) so internal-only lettering stays unique
 *  poem-wide, not just within this stanza.  Mutates in place. */
function attachInternalRhymes(lines: LineResult[], rhymes: LineRhyme[], allocator: KeyAllocator): void {
  ...
  // Key per class: a class containing a lettered end takes that end's key;
  // an internal-only class of >=2 draws a fresh poem-wide key from `allocator`.
  ...
  for (const [root, members] of classMembers) {
    ...
    else if (members.some(m => positions[m].kind === 'internal')) classLetter.set(root, allocator.next());
  }
...
export function applyRhymeAndForm(stanzas: LineResult[][]): void {
  // End-rhyme detection runs ONCE over the whole poem, flattened in reading
  // order — a rhyme sound keeps its key across a stanza break instead of
  // restarting at 'A', and matchedLine is a poem-global line index.  FORM
  // detection (couplet/quatrain/sonnet patterns) still reads per-stanza
  // SLICES of this one result: canonicalScheme remaps every slice to a fresh
  // sequential alphabet, so a stanza's pattern reads the same regardless of
  // which poem-wide keys its lines actually carry.
  const allLines = stanzas.flat();
  const globalRhymes = detectScheme(allLines.map(lineEndWord), allLines.map(l => lineEndTail(l)));
  let offset = 0;
  const analyzed = stanzas.map(lines => {
    const details = lines.map(l => l.phonologicalScansion);
    const rhymes = globalRhymes.slice(offset, offset + lines.length);
    offset += lines.length;
  ...
  const pform = poemForm(analyzed, globalRhymes);
  if (pform) for (const { details } of analyzed) for (const d of details) d.formNote = pform;
  // Poem-wide end-rhyme letters + TYPES (detectScheme), then the additive
  // pre-caesural internal-rhyme layer.  One allocator for the whole poem —
  // seeded with every end key already in use — so internal-only lettering
  // stays unique across stanza boundaries too (see attachInternalRhymes).
  // Writes `detail.rhyme`; the original end-rhyme display is restored and
  // extended, never replaced.
  const allocator = makeKeyAllocator(new Set(globalRhymes.filter(r => r.letter !== '·').map(r => r.letter)));
  for (const { lines, rhymes } of analyzed) attachInternalRhymes(lines, rhymes, allocator);
}
...
export function analyzePhonopoetics(stanzas: LineResult[][]): Phonopoetics {
  ...
  // Fresh keys for caesural/head-only groups: an allocator seeded with every
  // key the end scheme already uses, so caesural and head lettering (below)
  // never collides with an end group OR each other, at any poem length.
  const allocator = makeKeyAllocator(new Set(es.filter(r => r.letter !== '·').map(r => r.letter)));
  const freshLetter = (): string => allocator.next();
