...
import { buildFabbHalleGrid, type FHSyllable } from './fabbhalle.js';
...
export function fabbHalleSyllablesOf(result: LineResult): FHSyllable[] {
  const out: FHSyllable[] = [];
...
export function renderUnifiedDisplay(result: LineResult, rawLine?: string, opts?: RenderOptions): string {
  ...
  // engine's final meter name.  Never fed back into the main scansion.
  {
    ...
    // words stay lex 0 — clitics carry no lexical stress in F&H's sense.
    {
      ...
      for (const w of words) {
        ...
        for (const s of w.syllables) {
          if (k < fhSyls.length) fhSyls[k].lex = cliticMono ? 0 : (s.lexicalStress ?? s.stress ?? 0);
          k++;
        }
      }
    }
    let fh: ReturnType<typeof buildFabbHalleGrid>;
    let discoveredNote = '';
    if (opts?.fhSchema) {
      const size = opts.fhSchema === 'loose' ? 0
        : opts.fhSchema === 'iambic' || opts.fhSchema === 'trochaic' ? 2 : 3;
      fh = buildFabbHalleGrid(
        fhSyls, opts.fhSchema,
        size > 0 ? Math.floor(fhSyls.length / size) : undefined,
      );
